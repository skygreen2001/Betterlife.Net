...
namespace Util.Common
{
        ...
        #region 2.指定在注册表中存储值时所用的数据类型，或标识注册表中某个值的数据类型                
        ...
        /// </summary>
        /// 
        public static class UtilRegedit
        {
            /// <summary>
            /// 创建注册表键值
            /// </summary>
            /// <example>
            /// HKEY_CURRENT_USER\SOFTWARE\XibenCallcenter\core\AppPath
            /// </example>
            /// <param name="path">Registry Path路径</param>
            /// <param name="value">值</param>
            /// <returns></returns>
            public static void createKey(string PathKey, string value)
            {
                if (!string.IsNullOrEmpty(PathKey) && PathKey.Contains("\\"))
                {
                    string[] PathFragments = PathKey.Split('\\');
                    string rootPath = PathFragments[0];
                    string Path = "";
                    for (int i = 1; i < PathFragments.Length - 1; i++)
                    {
                        Path += PathFragments[i];
                        if (i < PathFragments.Length - 2)
                        {
                            Path += "\\";
                        }
                    }
                    ...
                    RegistryBase reg = new RegistryBase(Path, Root(rootPath));
                    reg.RegeditKey = key;
                    reg.WriteRegeditKey(value);
                    
                    //RegistryKey registryKey = Root(rootPath).CreateSubKey(Path);
            ...
            /// <summary>
            /// 根据RootPath创建对应的根RegistryKey
            /// </summary>
            /// <param name="RootPath"></param>
            public static RegDomain Root(string RootPath)
            {
                if (RootPath == "HKEY_CURRENT_USER")
                {
                    return RegDomain.CurrentUser;
                }
                else if (RootPath == "HKEY_CLASSES_ROOT")
                {
                    return RegDomain.ClassesRoot;
                }
                else if (RootPath == "HKEY_LOCAL_MACHINE")
                {
                    return RegDomain.LocalMachine;
                }
                else if (RootPath == "HKEY_USERS")
                {
                    return RegDomain.User;
                }
                else if (RootPath == "HEKY_CURRENT_CONFIG")
                {
                    return RegDomain.CurrentConfig;
                }
                else if (RootPath == "HKEY_PERFORMANCE_DATA")
                {
                    return RegDomain.PerformanceData;
                }
                //else if (RootPath == "HKEY_DYN_DATA")
                //{
        ...
        /// </summary>
        /// C#注册表操作类（完整版）
        /// <see cref="http://www.7747.net/kf/201007/52894.html"/>
         public class RegistryBase
         {
             ...
             public RegistryBase(string subKey, RegDomain regDomain)
             {
                 ///设置注册表项名称               
                 _subkey = subKey;
                 ///设置注册表基项域               
